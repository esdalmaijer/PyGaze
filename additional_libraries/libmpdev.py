...
import os
import time
from ctypes import byref, c_int, c_double, windll
...
class MP150(object):
    ...
    def __init__(self, logfile='default', samplerate=200):
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception('Error in libmpdev: '
                            'failed to connect to the MP150: %s' % result)
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception('Error in libmpdev: '
                            'failed to set samplerate: %s' % result)
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception('Error in libmpdev: '
                            'failed to set channels to acquire: %s' %  result)

        # start acquisition daemon (NOT for getMPBuffer and
        # getMostRecentSample!)
        try:
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception('Error in libmpdev: '
                            'failed to start acquisition daemon: %s' % result)
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception('Error in libmpdev: '
                            'failed to start acquisition: %s' % result)

        # open log file
        self._logfile = open(self._logfilename, 'w')
        # write header
        header = '\t'.join(['timestamp', 'channel_1', 'channel_2',
                            'channel_3'])
    ...
    def close(self):
        ...
        if check_returncode(result) != "MPSUCCESS":
            raise Exception(
                'Error in libmpdev: '
                'failed to close the connection to the MP150: %s' % result)
    ...
    def _sampleprocesser(self):
        ...
        while self._connected:
            ...
            if check_returncode(result) != "MPSUCCESS":
                raise Exception(
                    'Error in libmpdev: '
                    'failed to obtain a sample from the MP150: %s' % result)
            ...
            if self._recording:
                ...
                self._logfile.write(
                    '%d\t%.3f\t%.3f\t%.3f\n' % (self.get_timestamp(),
                                                self._newestsample[0],
                                                self._newestsample[1],
                                                self._newestsample[2]))
                # release the logging lock
                self._loglock.release()
