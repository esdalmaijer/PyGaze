...
if sys.platform == 'win32':
    ...
    # if this fails, we present a warning, but do not need to crash
        ...
        print ('WARNING! libwebcam: vidcap could not be imported; '
               'pygame back-end is still available')
...
try:
    ...
    if sys.platform == 'win32' and vidimp:
        print ('WARNING! libwebcam: pygame could not be imported; '
               'vidcap back-end is still available')
    # raise an exception if vidcap failed to load as well
    else:
        raise Exception('Error in libwebcam: '
                        'could not import and initialize PyGame camera module;'
                        ' could not import vidcap either!')
...
def available_devices():
    ...
    # if this fails, we try vidcap (only works on Windows)
        ...
        # if PyGame did not work and we're not on Windows, raise exception
            ...
            raise Exception('Error in libwebcam.available_devices: '
                            'could not use PyGame or vidcap to find devices!')
...
class Camera(object):
    ...
    def __init__(self, disptype=DISPTYPE, dev=None, devtype='pygame',
        ...
        # if we survived all the trouble shooting, finally set the devtype
        # property
        ...
        if self.devtype == 'vidcap':
            ...
            # first argument: devnr;
            # second argument: Boolean indicating if camimages should be
            # displayed on separate window (e.g. for debugging purposes)
            self.dev = vidcap.new_Dev(self.devname, 0)
    ...
    def set_imgflip(self, verflip=False, horflip=False):
        ...
        if self.devtype == 'vidcap' or self.disptype == 'psychopy':
            if self.verflip:
                self.flipnr = -1 # or 1?
            if self.horflip:
                print ('WARNING in libwebcam.Camera.set_imgflip: '
                       'horizontal flipping not supported (yet)')
        ...
        # PyGame camera and image modules
            ...
            try:
                ...
                print ('WARNING in libwebcam.Camera.set_imgflip: '
                       'flipping not supported by the device')
    ...
    def get_image(self):
        ...
        # other platforms
            ...
            # if the disptype is psychopy, we need to convert the PyGame image to a PIL Image
                ...
                self.img.fromstring(pygame.image.tostring(img, 'RGB', False),
                                    'raw', 'RGB', 0, self.flipnr)
                return self.img
