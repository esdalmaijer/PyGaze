...
class PyGameSound(BaseSound):
    ...
    # see pygaze._sound.basesound.BaseSound
        ...
        def __init__(self, osc=settings.SOUNDOSCILLATOR,
            ...
            # see pygaze._sound.basesound.BaseSound
                ...
                if soundfile != None:
                    ...
                    slen = int(settings.SOUNDSAMPLINGFREQUENCY * length / 1000) # number of samples
        ...
        def pan(self, panning):
            ...
            if isinstance(panning, (int, float)) and panning < -1:
                panning = -1
            elif isinstance(panning, (int, float)) and panning > 1:
                panning = 1

            # round off, to prevent too long numbers
            if isinstance(panning, float):
            ...
            for i in range(len(buf)):
                ...
                if panning == 'left':
                    ...
                    r = float(r) / abs(panning)
                elif panning > 0:
                    l = float(l) / abs(panning)
